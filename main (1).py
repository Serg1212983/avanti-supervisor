...
Критерії: середній+ сегмент, маржа 30%+, є ексклюзивний імпортер або можливість стати дистриб'ютором.
Для кожного бренду:
• Назва та країна походження
• Категорія (волосся/нігті/обличчя)
• Чому підходить для AVANTI
• Як знайти імпортера/дистриб'ютора
• Орієнтовна маржа
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != OWNER_CHAT_ID:
        return
    now = kyiv_time()
    web_status = "✅ Веб-пошук активний (5 паралельних запитів)" if TAVILY_KEY else "❌ Веб-пошук не підключений"
...
async def cmd_agent1(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "🔍 Агент №1 запущено!\n"
        "Виконую 5 паралельних пошуків в інтернеті...\n"
...
def main():
    ...
    print("🚀 AVANTI Supervisor запущений! Агент №1: 5 паралельних пошуків!")
